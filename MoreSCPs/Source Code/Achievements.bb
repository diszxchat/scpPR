...
;More SCPS Mod goes here

Const Achv005% = 37, Achv409% = 38, Achv1074% = 39, Achv009% = 40, Achv038% = 41, Achv020% = 42
...
For i = 0 To MAXACHIEVEMENTS-1
	Local loc2% = GetINISectionLocation("MoreSCPs\Data\achievementstrings.ini", "s"+Str(i))
	AchievementStrings(i) = GetINIString2("MoreSCPs\Data\achievementstrings.ini", loc2, "string1")
	AchievementDescs(i) = GetINIString2("MoreSCPs\Data\achievementstrings.ini", loc2, "AchvDesc")
	
	Local image$ = GetINIString2("MoreSCPs\Data\achievementstrings.ini", loc2, "image") 
	
	AchvIMG(i) = LoadImage_Strict("MoreSCPs\GFX\menu\achievements\"+image+".jpg")
	AchvIMG(i) = ResizeImage2(AchvIMG(i),ImageWidth(AchvIMG(i))*GraphicHeight/768.0,ImageHeight(AchvIMG(i))*GraphicHeight/768.0)
Next

Global AchvLocked = LoadImage_Strict("MoreSCPs\GFX\menu\achievements\achvlocked.jpg")
...
Function GiveAchievement(achvname%, showMessage%=True)
	If Achievements(achvname)<>True Then
		...
		If AchvMSGenabled And showMessage Then
			Local loc2% = GetINISectionLocation("MoreSCPs\Data\achievementstrings.ini", "s"+achvname)
			Local AchievementName$ = GetINIString2("MoreSCPs\Data\achievementstrings.ini", loc2, "string1")
