...
Function UpdateMainMenu()
	...
	Else
		...
		Else If MainMenuTab<>5
			Color 0,0,0
			Text (GraphicWidth/2)+(420*MenuScale)+(3*MenuScale)-StringWidth("BACK"),y+(3*MenuScale),"BACK",False,False
			If MouseOn((GraphicWidth/2)+(420*MenuScale)-StringWidth("BACK"), y, StringWidth("BACK"), StringHeight("BACK")) Then
				Color 100,100,150
				If MouseHit1 Then back = True : PlaySound(ButtonSFX)
			Else
				Color 255,255,255
			EndIf
			
			Text (GraphicWidth/2)+(420*MenuScale)-StringWidth("BACK"),y,"BACK",False,False
		EndIf
		...
		If back Then 
			Select MainMenuTab
				Case 3 ;save the options
					...
					PutINIValue("BoxOfHorrors\"+OptionFile, "options", "camera fog scale", CameraFogScale)
		...
		Select MainMenuTab
			Case 1 ; New game
				...
				Else
					Text(GraphicWidth/2, y + height / 2, "NEW GAME", True, True)
				EndIf
